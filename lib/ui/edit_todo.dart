...
Future editTodoPage(BuildContext context, int key, String todoTitle, String todoDate) async {
  DateTime date = DateFormat('yyyy-MM-dd').parse(todoDate);
  String format = DateFormat('yyyy-MM-dd').format(date);

  var repository = ToDoRepositoryImpl();
  MainViewModel mainViewModel = MainViewModel(repository: repository);
  final _textController = TextEditingController(text: todoTitle);
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  IconButton(
                    onPressed: () async {
                      // "일정을 수정할까요?" 라는 메세지와 버튼이 있는 AlertDialog
                      var result = await showDialog<bool>(
                        context: context,
                        builder: (context) {
                          return AlertDialog(
                            title: const Text('일정을 수정할까요?'),
                            actions: [
                              TextButton(
                                onPressed: () {
                      ...
                      if (result == true) {
                        await mainViewModel.editTodo(key, _textController.text, format);
                        Navigator.of(context).pop(); // 닫기
                      }
                    },
